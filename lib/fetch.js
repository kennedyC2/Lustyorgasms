		...
		.then((json) => {
			...
			for (const d of videoData) {
				...
				data += `<div class="outer-wrapper v${(count += 1)}">
                        ...
                        <a href="playVideo?id=${d.video.video_id}"><img data-src="${d.video.thumb}" alt="thumbnail" width="100%" height="100%"/></a>
...
fetchData.pornStarVideos = function (name, page, callback) {
	// Instantiate request
	fetch("https://api.redtube.com/?data=redtube.Videos.searchVideos&output=json&stars[]=" + name + "&thumbsize=big&page=" + page)
		.then((data) => data.json())
		.then((json) => {
			var videoData = json.videos;
			var num = json.count;
	...
	fetch("https://embed.redtube.com/?id=" + id)
		...
		.then((text) => {
			...
			for (const prop of hell2) {
				prop.replace("'", "");
				var ned = prop.replace('"', "").replace('"', "").replace('"', "").replace('"', "").replace('"', "").replace('"', "").split(":");
				if (ned[0] === "image_url") {
					imageThumb =
						ned[1] +
						":" +
