...
fetchData.listVideos = (page, callback) => {
	let count = 0;

	// Instantiate request
	fetch(`https://api.redtube.com/?data=redtube.Videos.searchVideos&output=json&search=hard&ordering=${videoCategories[0]}&thumbsize=big&page=` + page, (err, response) => {
		if (!err && response) {
			var videoData = response.videos;
			...
			callback(false, data, videoCategories[0]);
		} else {
			console.log(err);
		}
	});
...
fetchData.search = (word, page, callback) => {
	// Instantiate request
	fetch(`https://api.redtube.com/?data=redtube.Videos.searchVideos&output=json&search=${word}&ordering=${videoCategories[0]}&thumbsize=big&page=` + page, (err, response) => {
		if (!err && response) {
			...
			// Return videos
			callback(false, videos, videoCategories[0]);
		} else {
			console.log(err);
		}
	});
...
fetchData.videoByTags = (str, page, callback) => {
	...
	translator(_tag, (err, tag) => {
		if (!err && tag) {
			// Instantiate Request
			fetch(`https://api.redtube.com/?data=redtube.Videos.searchVideos&output=json&search=${tag}&tags[]=${tag}&ordering=${videoCategories[0]}&thumbsize=big&page=` + page, (err, response) => {
				if (!err && response) {
					// Check if video exists
					// If video does not exist, return error
					if (!response.videos) {
