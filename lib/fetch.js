...
// Translator
fetchData.translate = function (trans, callback) {
...
// ===================================================================================
fetchData.videoByTags = function (trans, page, callback) {
	// Check query
	var tran = trans == "180° Virtual Reality" ? "Virtual Reality" : trans;

	fetchData.translate(tran, function (err, tag) {
		if (!err && tag) {
			// Instantiate Request
	...
	fetch("https://embed.redtube.com/related?id=" + id)
		...
		.then((json) => {
			...
			for (const d of videoData) {
				...
				data += `<div class="outer-wrapper v${(count += 1)}">
                        <div class="thumb-wrapper">
                        <a href="view/${videoId[3]}"><img data-src="${d[0]}" alt="thumbnail"/></a>
